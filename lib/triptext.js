...
function formatStopText(row, rev) {
  ...
  if (row.addr) out.push(row.addr);
  out.push(row.lat.toFixed(5) + ', ' + row.lon.toFixed(5));
  // rev is ATLAS_REV, which already reads "Atlas Rev 08-2026". Prefixing
  // another "Atlas " here is how this text said "Atlas Atlas Rev 08-2026"
  // for eleven releases before anyone read their own share message.
  out.push(rev || '');
  return out.join('\n');
}
