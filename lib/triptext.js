...
function formatStopText(row, rev) {
  ...
  // Coordinates, deliberately, even though rows now carry a street address.
  // The stop sheet swapped to the address because that is what a driver reads
  // off the screen; this text is pasted into other things, and a coordinate
  // pair is the one form that drops into any nav app or dispatch field
  // without being re-geocoded into an approximation of itself.
