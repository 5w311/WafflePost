...
function formatStopText(row, rev) {
  ...
  // The Waffle House's address rather than its coordinates: this text gets
  // read aloud over a phone and pasted into nav apps, and neither of those
  // works with a decimal pair. Rows without one fall back to coordinates so
  // the line is never simply missing.
  out.push(row.addr || (row.lat.toFixed(5) + ', ' + row.lon.toFixed(5)));
  out.push('Atlas ' + (rev || ''));
  return out.join('\n');
}
